...
save(silent, singles, singles_stops, file="nscore_df_mRIP1.RData")

################################################################################
# Check how many variants are present in the input and in the output
variant_data_merge<-read_tsv("mRIP1_variant_data_merge.tsv")
...
library("writexl")
write_xlsx(singles_stops, "mRIP1_singles.xlsx")
write_xlsx(silent, "mRIP1_synonymous.xlsx")

write.csv(singles_stops, "mRIP1_singles.csv", row.names=FALSE)
write.csv(silent, "mRIP1_synonymous.csv", row.names=FALSE)
