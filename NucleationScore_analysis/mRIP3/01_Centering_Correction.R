...
ggsave(p_iqr, file="p_iqr.jpg", path=path, width = 4, height = 4)
...
save(silent, singles, singles_stops, file="nscore_df_mRIP3.RData")

################################################################################
# Check how many variants are present in the input and in the output
variant_data_merge<-read_tsv("mRIP3_variant_data_merge.tsv")
...
library("writexl")
write_xlsx(singles_stops, "mRIP3_singles.xlsx")
write_xlsx(silent, "mRIP3_synonymous.xlsx")

write.csv(singles_stops, "mRIP3_singles.csv", row.names=FALSE)
write.csv(silent, "mRIP3_synonymous.csv", row.names=FALSE)
