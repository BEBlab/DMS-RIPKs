...
#############################################################################################################
# Individual Validation: nucleation score vs growth rates

individual_validation<-read_tsv("RIPK1_IndividualValidation.txt")

individual_validation<-individual_validation %>% group_by(ID) %>% summarise(mean_growth=mean(growth_rate), 
                                                                            std_growth=sd(growth_rate))

# RIPK1 individual validation:

individual_validation<-left_join(individual_validation, singles_stops[c("ID", "nscore_c", "sigma")], by="ID")

RIPK1<-data.frame("ID"="RIPK1", "mean_growth" = 23.5, "std_growth" = 3.6, "nscore_c"=0.115299498, "sigma"= 0.06311079)

individual_validation<-rbind(individual_validation, RIPK1)
